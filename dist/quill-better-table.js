(function webpackUniversalModuleDefinition(root, factory) {
  if (typeof exports === "object" && typeof module === "object") module.exports = factory(require("quill"));
  else if (typeof define === "function" && define.amd) define(["quill"], factory);
  else if (typeof exports === "object") exports["quillBetterTable"] = factory(require("quill"));
  else root["quillBetterTable"] = factory(root["Quill"]);
  ...
  return /******/ (function (modules) {
    ...
    /******/ [
      ...
      /***/ function (module, __webpack_exports__, __webpack_require__) {
        ...
        var external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default = /*#__PURE__*/ __webpack_require__.n(
          external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_
        );
        ...
        class header_Header extends Block {
          ...
          format(name, value) {
            const { row, cell, rowspan, colspan } = header_Header.formats(this.domNode);
          ...
          optimize(context) {
            const { row, rowspan, colspan } = header_Header.formats(this.domNode);
        ...
        class table_selection_TableSelection {
          ...
          correctBoundary() {
            ...
            tableCells.forEach((tableCell) => {
              let { x, y, width, height } = getRelativeRect(tableCell.domNode.getBoundingClientRect(), this.quill.root.parentNode);
              let isCellIntersected =
                ((x + table_selection_ERROR_LIMIT >= this.boundary.x && x + table_selection_ERROR_LIMIT <= this.boundary.x1) ||
                  (x - table_selection_ERROR_LIMIT + width >= this.boundary.x && x - table_selection_ERROR_LIMIT + width <= this.boundary.x1)) &&
                ((y + table_selection_ERROR_LIMIT >= this.boundary.y && y + table_selection_ERROR_LIMIT <= this.boundary.y1) ||
                  (y - table_selection_ERROR_LIMIT + height >= this.boundary.y && y - table_selection_ERROR_LIMIT + height <= this.boundary.y1));
          ...
          computeSelectedTds() {
            ...
            return tableCells.reduce((selectedCells, tableCell) => {
              let { x, y, width, height } = getRelativeRect(tableCell.domNode.getBoundingClientRect(), this.quill.root.parentNode);
              let isCellIncluded =
                x + table_selection_ERROR_LIMIT >= this.boundary.x &&
                x - table_selection_ERROR_LIMIT + width <= this.boundary.x1 &&
                y + table_selection_ERROR_LIMIT >= this.boundary.y &&
                y - table_selection_ERROR_LIMIT + height <= this.boundary.y1;
          ...
          insertColumnRight: {
            ...
            handler() {
              ...
              this.quill.setSelection(
                ...
                0,
                external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.sources.SILENT
              );
          ...
          mergeCells: {
            ...
            handler() {
              ...
              const rowspan = tableContainer.rows().reduce((sum, row) => {
                ...
                if (
                  rowRect.y > this.boundary.y - table_operation_menu_ERROR_LIMIT &&
                  rowRect.y + rowRect.height < this.boundary.y + this.boundary.height + table_operation_menu_ERROR_LIMIT
                ) {
                  sum += 1;
                }
              ...
              const colspan = this.columnToolCells.reduce((sum, cell) => {
                ...
                if (
                  cellRect.x > this.boundary.x - table_operation_menu_ERROR_LIMIT &&
                  cellRect.x + cellRect.width < this.boundary.x + this.boundary.width + table_operation_menu_ERROR_LIMIT
                ) {
                  sum += 1;
                }
          ...
          deleteColumn: {
            ...
            handler() {
              ...
              let colIndexes = getColToolCellIndexesByBoundary(
                ...
                (cellRect, boundary) => {
                  return (
                    cellRect.x + table_operation_menu_ERROR_LIMIT > boundary.x && cellRect.x + cellRect.width - table_operation_menu_ERROR_LIMIT < boundary.x1
                  );
                },
                this.quill.root.parentNode
              );
        ...
        class table_operation_menu_TableOperationMenu {
          ...
          menuItemCreator(_ref2) {
            let { text, iconSrc, handler } = _ref2;
            const node = document.createElement("div");
          ...
          "table-cell-line enter": {
            ...
            handler(range, context) {
              ...
              this.quill.insertText(
                ...
                external_commonjs_quill_commonjs2_quill_amd_quill_root_Quill_default.a.sources.USER
